...
- **Решение (Reverse Proxy HTTP 200 OK)**: Vercel привязывается к кастомному домену (`sub.silentconnect.net` / `silentconnect.net`) и выступает **полным обратным прокси (посредником)**. Vercel сам за миллисекунды забирает данные с Hetzner FI (или NL) и отдаёт клиенту Happ чистый `HTTP 200 OK` с первого раза со всеми заголовками (`subscription-userinfo`, `Content-Type: text/plain`).
...
- Для пользователя в адресной строке остаётся **`https://silentconnect.net`**.
- В приложении Happ адрес подписки остаётся **`https://sub.silentconnect.net/...`**.
...
🟢 1. ШТАТНЫЙ РЕЖИМ (99.9% времени):
     • Прямой трафик на NL Master (193.233.210.189).
...
🔴 2. АВАРИЙНЫЙ РЕЖИМ (Падение NL):
     ...
     • Vercel Anycast мгновенно пробивает ТСПУ/DPI и исключает ожидание DNS-кэша провайдеров в РФ, 
       направляя трафик покупателей на FI (95.217.178.48).
