...
class TestPlategaSecurity(unittest.TestCase):
    def test_security_scanner_detects_platega_secret(self):
        ...
        try:
            ...
            self.assertEqual(result.returncode, 1, f"Expected scanner to fail with 1, got {result.returncode}")
            self.assertTrue("SEC-010" in result.stdout or "Platega" in result.stdout, f"Expected SEC-010 violation in output: {result.stdout}")
        finally:
            if tmp_file.exists():
                tmp_file.unlink()

    def test_gitignore_blocks_env_platega(self):
        github_export_dir = _find_export_dir()
        ...
        try:
            ...
            self.assertNotIn(".env.platega", res.stdout, f".env.platega should be ignored by git: {res.stdout}")
        finally:
            if test_env.exists():
                test_env.unlink()
