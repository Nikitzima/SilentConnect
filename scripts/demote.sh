#!/usr/bin/env bash
# ==============================================================================
# demote.sh - Generalized Safe Failback & Re-synchronization Script
# Supports both FI (Finland) and PL (Poland) standby nodes.
#
# Usage:
#   demote.sh --node fi    # Demote FI back to NL (legacy: demote_fi.sh still works)
#   demote.sh --node pl    # Demote PL back to NL (new)
#   demote.sh --node=pl    # Alternate syntax
#
# Failback steps:
# 1. Acquires mutual exclusion flock (/var/run/demote_<node>.lock).
# 2. Performs 30-second stability/debounce check on NL.
# 3. Initiates Quiesce Lock on standby SubJSON (30s lease + 10s heartbeat loop).
# 4. Stops vpn-shop-silentconnect and vpn-shop-web services on standby.
...
# 8. Releases standby Quiesce Lock.
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --node=*)
            ...
            ;;
        *)
            echo "[demote] Unknown argument: $1" >&2
            exit 1
            ;;
    esac
done
...
if [ "$NODE" != "fi" ] && [ "$NODE" != "pl" ]; then
    echo "[demote] ERROR: --node must be 'fi' or 'pl', got '$NODE'" >&2
    exit 1
fi
...
case "$NODE" in
        ...
        LOCK_FILE="${LOCK_FILE:-/var/run/demote_fi.lock}"
        ...
        ;;
    pl)
        LOCK_FILE="${LOCK_FILE:-/var/run/demote_pl.lock}"
        STATE_FILE="${STATE_FILE:-/var/run/cluster_state}"
        NL_IP="${NL_IP:-${NL_MASTER_IP:-193.233.210.189}}"
...
exec 200>"$LOCK_FILE"
if ! flock -n 200; then
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] [demote_${NODE}] ERROR: Another promotion/demotion process is currently running." >&2
    exit 1
fi

log() {
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] [demote_${NODE}] $*"
}

err() {
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] [demote_${NODE}] ERROR: $*" >&2
}
...
# Step 2: Quiesce Lock on standby SubJSON
log "Step 2: Activating Quiesce Lock on ${NODE_LABEL} SubJSON..."
...
# Step 3: Stop vpn-shop services on standby node
log "Step 3: Stopping vpn-shop services on ${NODE_LABEL}..."
systemctl stop vpn-shop-silentconnect.service vpn-shop-web.service 2>/dev/null || true

# Step 4: Transfer standby DBs and Baselines to NL and execute failback_merge.py
log "Step 4: Executing 3-Way Data Merge on NL..."
ssh -o BatchMode=yes -o ConnectTimeout=10 root@${NL_IP} "mkdir -p /tmp/${NODE}_merge"

# Copy standby databases to NL merge staging
scp -o BatchMode=yes /etc/x-ui/x-ui.db root@${NL_IP}:/tmp/${NODE}_merge/${NODE}_xui.db
scp -o BatchMode=yes /root/vpn-shop/data-silentconnect/vpn_shop.db root@${NL_IP}:/tmp/${NODE}_merge/${NODE}_vpn.db

if [ -f "/var/lib/litestream/baseline_xui.db" ]; then
    scp -o BatchMode=yes /var/lib/litestream/baseline_xui.db root@${NL_IP}:/tmp/${NODE}_merge/baseline_xui.db
fi
if [ -f "/var/lib/litestream/baseline_vpn_shop.db" ]; then
    scp -o BatchMode=yes /var/lib/litestream/baseline_vpn_shop.db root@${NL_IP}:/tmp/${NODE}_merge/baseline_vpn_shop.db
fi

# Run failback_merge.py on NL
# pass --standby-node so the merge script knows the source node
ssh -o BatchMode=yes -o ConnectTimeout=30 root@${NL_IP} "python3 /usr/local/bin/failback_merge.py \
    ...
    --standby-vpn /tmp/${NODE}_merge/${NODE}_vpn.db \
    --baseline-vpn /tmp/${NODE}_merge/baseline_vpn_shop.db \
    --nl-xui /etc/x-ui/x-ui.db \
    --standby-xui /tmp/${NODE}_merge/${NODE}_xui.db \
    --baseline-xui /tmp/${NODE}_merge/baseline_xui.db"
...
if [ -x "/usr/local/bin/cf-failover-dns.sh" ]; then
    /usr/local/bin/cf-failover-dns.sh "demote-${NODE}"
else
...
# Step 7: Release Quiesce Lock on standby node
log "Step 7: Releasing Quiesce Lock on ${NODE_LABEL}..."
cleanup_heartbeat
HEARTBEAT_PID=""
...
if [ -n "$BOT_TOKEN" ] && [ -n "$ADMIN_ID" ]; then
    MSG="✅ <b>[FAILBACK COMPLETE]</b>%0A%0ASilentConnect cluster returned to <b>NL Master (${NL_IP})</b>.%0A%0A• 3-Way Data Merge: Successful.%0A• NL Services: Active & Verified.%0A• DNS: Switched to NL (DNS-Only ⚪).%0A• ${NODE_LABEL} Quiesce: Released."
